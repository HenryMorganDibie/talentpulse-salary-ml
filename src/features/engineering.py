...
- All transformations are stateless or fit on train only and applied to test.
...
# Ordered seniority levels used for one-hot encoding
...
def add_seniority_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Captures the 45-80% salary premium documented in TalentPulse business brief.
    """
    df = df.copy()
    senior_mask = df.get("job_level", pd.Series(dtype=str)).isin(["Senior", "Lead"])
...
def add_experience_tier(df: pd.DataFrame) -> pd.DataFrame:
    """
    Bucket years-of-experience into ordinal tiers:
        0  → 0–2 yrs  (entry level)
        1  → 3–5 yrs  (mid level)   [default when missing]
        2  → 6–9 yrs  (senior)
...
def add_skill_flags(df: pd.DataFrame, skills: List[str] = TOP_SKILLS) -> pd.DataFrame:
    """
    Binary flag for each high-value skill.
    Captures scarcity premium for Python, SQL, Spark, AWS.
    Requires the 'skills_list' column (parsed list of strings).
    """
    df = df.copy()
    if "skills_list" not in df.columns:
        raise ValueError("Run parse_skills_column() before add_skill_flags()")

    for sk in skills:
        df[f"skill_{sk}"] = df["skills_list"].apply(
            lambda lst: int(any(sk.lower() in str(item).lower() for item in lst))
        )
    return df
...
def add_remote_flag(df: pd.DataFrame) -> pd.DataFrame:
    """Cast boolean is_remote to integer for modelling."""
    df = df.copy()
...
def one_hot_encode(
    ...
    """One-hot encode categorical columns, preserving all categories."""
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Full feature engineering pipeline. Call this on the labelled dataset
    after parse_skills_column().
...
def get_feature_columns(df: pd.DataFrame) -> List[str]:
    """
    Return the ordered list of model features from an engineered DataFrame.
    Dynamically picks up OHE columns so it stays correct if the dataset
    gains new countries or levels.
    """
    base = [
        "num_skills",
        "experience_tier",
